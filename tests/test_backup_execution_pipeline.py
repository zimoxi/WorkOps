"""
WorkOps Backup Execution Pipeline Tests
Sprint053: Backup Execution Pipeline Foundation

覆盖：
- BackupExecutionStatus enum
- BackupExecutionRequest validation
- BackupExecutionResult validation
- BackupExecutor contract
- BackupExecutionPipeline contract
- validate_backup_execution_request
- Error model
- Security boundary
"""

import unittest
from datetime import datetime, timezone

from backup_manager.backup_execution.model import (
    BackupExecutionStatus,
    BackupExecutionRequest,
    validate_backup_execution_request,
)
from backup_manager.backup_execution.result import BackupExecutionResult
from backup_manager.backup_execution.executor import BackupExecutor
from backup_manager.backup_execution.pipeline import BackupExecutionPipeline
from backup_manager.backup_execution.errors import (
    BackupExecutionError,
    InvalidBackupExecutionRequestError,
    BackupExecutionConflictError,
    BackupExecutionUnavailableError,
)


# ============================================================================
# BackupExecutionStatus
# ============================================================================

class TestBackupExecutionStatus(unittest.TestCase):
    """备份执行状态测试"""

    def test_created(self):
        self.assertEqual(BackupExecutionStatus.CREATED.value, "created")

    def test_preparing(self):
        self.assertEqual(BackupExecutionStatus.PREPARING.value, "preparing")

    def test_running(self):
        self.assertEqual(BackupExecutionStatus.RUNNING.value, "running")

    def test_completed(self):
        self.assertEqual(BackupExecutionStatus.COMPLETED.value, "completed")

    def test_failed(self):
        self.assertEqual(BackupExecutionStatus.FAILED.value, "failed")

    def test_five_statuses(self):
        self.assertEqual(len(BackupExecutionStatus), 5)

    def test_invalid_rejected(self):
        with self.assertRaises(ValueError):
            BackupExecutionStatus("nonexistent")


# ============================================================================
# BackupExecutionRequest
# ============================================================================

class TestBackupExecutionRequest(unittest.TestCase):
    """备份执行请求测试"""

    def _make_request(self, **kwargs):
        defaults = {
            "backup_id": "b-001",
            "operation_id": "op-001",
            "job_id": "j-001",
            "execution_id": "exec-001",
            "adapter_id": "linux-001",
        }
        defaults.update(kwargs)
        return BackupExecutionRequest(**defaults)

    def test_valid_request(self):
        req = self._make_request()
        self.assertEqual(req.backup_id, "b-001")
        self.assertEqual(req.operation_id, "op-001")
        self.assertEqual(req.job_id, "j-001")
        self.assertEqual(req.execution_id, "exec-001")
        self.assertEqual(req.adapter_id, "linux-001")

    def test_frozen(self):
        req = self._make_request()
        with self.assertRaises(AttributeError):
            req.backup_id = "other"

    def test_slots(self):
        req = self._make_request()
        with self.assertRaises(AttributeError):
            req.__dict__

    def test_empty_backup_id_rejected(self):
        with self.assertRaises(InvalidBackupExecutionRequestError):
            self._make_request(backup_id="")

    def test_empty_operation_id_rejected(self):
        with self.assertRaises(InvalidBackupExecutionRequestError):
            self._make_request(operation_id="")

    def test_empty_job_id_rejected(self):
        with self.assertRaises(InvalidBackupExecutionRequestError):
            self._make_request(job_id="")

    def test_empty_execution_id_rejected(self):
        with self.assertRaises(InvalidBackupExecutionRequestError):
            self._make_request(execution_id="")

    def test_empty_adapter_id_rejected(self):
        with self.assertRaises(InvalidBackupExecutionRequestError):
            self._make_request(adapter_id="")

    def test_timezone_aware(self):
        req = self._make_request()
        self.assertIsNotNone(req.created_at.tzinfo)

    def test_no_forbidden_fields(self):
        req = self._make_request()
        for attr in ["password", "credential", "secret", "token", "ssh", "command"]:
            self.assertFalse(hasattr(req, attr))

    def test_repr_no_secrets(self):
        req = self._make_request()
        r = repr(req)
        for term in ["password", "secret", "token", "credential"]:
            self.assertNotIn(term, r.lower())


# ============================================================================
# BackupExecutionResult
# ============================================================================

class TestBackupExecutionResult(unittest.TestCase):
    """备份执行结果测试"""

    def test_valid_result(self):
        result = BackupExecutionResult(
            backup_id="b-001", status=BackupExecutionStatus.COMPLETED,
            success=True, message="ok",
        )
        self.assertEqual(result.backup_id, "b-001")
        self.assertTrue(result.success)
        self.assertEqual(result.status, BackupExecutionStatus.COMPLETED)

    def test_frozen(self):
        result = BackupExecutionResult(
            backup_id="b-001", status=BackupExecutionStatus.COMPLETED,
            success=True, message="ok",
        )
        with self.assertRaises(AttributeError):
            result.backup_id = "other"

    def test_slots(self):
        result = BackupExecutionResult(
            backup_id="b-001", status=BackupExecutionStatus.COMPLETED,
            success=True, message="ok",
        )
        with self.assertRaises(AttributeError):
            result.__dict__

    def test_empty_backup_id_rejected(self):
        with self.assertRaises(InvalidBackupExecutionRequestError):
            BackupExecutionResult(
                backup_id="", status=BackupExecutionStatus.COMPLETED,
                success=True, message="ok",
            )

    def test_invalid_status_rejected(self):
        with self.assertRaises(InvalidBackupExecutionRequestError):
            BackupExecutionResult(
                backup_id="b-001", status="completed",
                success=True, message="ok",
            )

    def test_success_must_be_bool(self):
        with self.assertRaises(InvalidBackupExecutionRequestError):
            BackupExecutionResult(
                backup_id="b-001", status=BackupExecutionStatus.COMPLETED,
                success=1, message="ok",
            )

    def test_message_must_be_str(self):
        with self.assertRaises(InvalidBackupExecutionRequestError):
            BackupExecutionResult(
                backup_id="b-001", status=BackupExecutionStatus.COMPLETED,
                success=True, message=123,
            )

    def test_timezone_aware(self):
        result = BackupExecutionResult(
            backup_id="b-001", status=BackupExecutionStatus.COMPLETED,
            success=True, message="ok",
        )
        self.assertIsNotNone(result.completed_at.tzinfo)

    def test_all_statuses(self):
        for status in BackupExecutionStatus:
            result = BackupExecutionResult(
                backup_id="b-001", status=status,
                success=True, message="ok",
            )
            self.assertEqual(result.status, status)

    def test_no_forbidden_fields(self):
        result = BackupExecutionResult(
            backup_id="b-001", status=BackupExecutionStatus.COMPLETED,
            success=True, message="ok",
        )
        for attr in ["stdout", "stderr", "command", "secret", "credential", "token"]:
            self.assertFalse(hasattr(result, attr))

    def test_failed_result(self):
        result = BackupExecutionResult(
            backup_id="b-001", status=BackupExecutionStatus.FAILED,
            success=False, message="error",
        )
        self.assertFalse(result.success)
        self.assertEqual(result.status, BackupExecutionStatus.FAILED)


# ============================================================================
# BackupExecutor Contract
# ============================================================================

class TestBackupExecutorContract(unittest.TestCase):
    """备份执行器契约测试"""

    def test_is_abc(self):
        from abc import ABC
        self.assertTrue(issubclass(BackupExecutor, ABC))

    def test_has_execute(self):
        self.assertTrue(hasattr(BackupExecutor, "execute"))

    def test_cannot_instantiate(self):
        with self.assertRaises(TypeError):
            BackupExecutor()

    def test_concrete_subclass(self):
        class MockExecutor(BackupExecutor):
            def execute(self, request):
                return BackupExecutionResult(
                    backup_id=request.backup_id,
                    status=BackupExecutionStatus.COMPLETED,
                    success=True, message="ok",
                )
        executor = MockExecutor()
        req = BackupExecutionRequest(
            backup_id="b-001", operation_id="op-001",
            job_id="j-001", execution_id="exec-001", adapter_id="linux-001",
        )
        result = executor.execute(req)
        self.assertTrue(result.success)

    def test_missing_execute(self):
        class BadExecutor(BackupExecutor):
            pass
        with self.assertRaises(TypeError):
            BadExecutor()


# ============================================================================
# BackupExecutionPipeline Contract
# ============================================================================

class TestBackupExecutionPipelineContract(unittest.TestCase):
    """备份执行管道契约测试"""

    def test_is_abc(self):
        from abc import ABC
        self.assertTrue(issubclass(BackupExecutionPipeline, ABC))

    def test_has_prepare(self):
        self.assertTrue(hasattr(BackupExecutionPipeline, "prepare"))

    def test_has_execute(self):
        self.assertTrue(hasattr(BackupExecutionPipeline, "execute"))

    def test_cannot_instantiate(self):
        with self.assertRaises(TypeError):
            BackupExecutionPipeline()

    def test_concrete_subclass(self):
        class MockPipeline(BackupExecutionPipeline):
            def prepare(self, request):
                pass
            def execute(self, request):
                return BackupExecutionResult(
                    backup_id=request.backup_id,
                    status=BackupExecutionStatus.COMPLETED,
                    success=True, message="ok",
                )
        pipeline = MockPipeline()
        req = BackupExecutionRequest(
            backup_id="b-001", operation_id="op-001",
            job_id="j-001", execution_id="exec-001", adapter_id="linux-001",
        )
        pipeline.prepare(req)
        result = pipeline.execute(req)
        self.assertTrue(result.success)

    def test_missing_prepare(self):
        class BadPipeline(BackupExecutionPipeline):
            def execute(self, request):
                pass
        with self.assertRaises(TypeError):
            BadPipeline()

    def test_missing_execute(self):
        class BadPipeline(BackupExecutionPipeline):
            def prepare(self, request):
                pass
        with self.assertRaises(TypeError):
            BadPipeline()


# ============================================================================
# validate_backup_execution_request
# ============================================================================

class TestValidateBackupExecutionRequest(unittest.TestCase):
    """验证备份执行请求测试"""

    def test_valid_request(self):
        req = BackupExecutionRequest(
            backup_id="b-001", operation_id="op-001",
            job_id="j-001", execution_id="exec-001", adapter_id="linux-001",
        )
        validate_backup_execution_request(req)

    def test_invalid_type_rejected(self):
        with self.assertRaises(InvalidBackupExecutionRequestError):
            validate_backup_execution_request("not_a_request")


# ============================================================================
# Error Model
# ============================================================================

class TestBackupExecutionErrors(unittest.TestCase):
    """错误模型测试"""

    def test_execution_error(self):
        with self.assertRaises(BackupExecutionError):
            raise BackupExecutionError("test")

    def test_invalid_request_error(self):
        with self.assertRaises(BackupExecutionError):
            raise InvalidBackupExecutionRequestError("test")

    def test_conflict_error(self):
        exc = BackupExecutionConflictError("b-001")
        self.assertIn("b-001", str(exc))

    def test_unavailable_error(self):
        with self.assertRaises(BackupExecutionError):
            raise BackupExecutionUnavailableError("test")

    def test_error_messages_no_secrets(self):
        for exc_cls, args in [
            (BackupExecutionError, ("test",)),
            (InvalidBackupExecutionRequestError, ("test",)),
            (BackupExecutionConflictError, ("b-001",)),
            (BackupExecutionUnavailableError, ("test",)),
        ]:
            msg = str(exc_cls(*args))
            for term in ["password", "secret", "token", "credential", "ssh"]:
                self.assertNotIn(term, msg.lower())


# ============================================================================
# Security Boundary
# ============================================================================

class TestSecurityBoundary(unittest.TestCase):
    """安全边界测试"""

    def test_request_no_credentials(self):
        req = BackupExecutionRequest(
            backup_id="b-001", operation_id="op-001",
            job_id="j-001", execution_id="exec-001", adapter_id="linux-001",
        )
        for attr in ["password", "credential", "secret", "token", "ssh", "command"]:
            self.assertFalse(hasattr(req, attr))

    def test_result_no_credentials(self):
        result = BackupExecutionResult(
            backup_id="b-001", status=BackupExecutionStatus.COMPLETED,
            success=True, message="ok",
        )
        for attr in ["stdout", "stderr", "command", "secret", "credential", "token"]:
            self.assertFalse(hasattr(result, attr))

    def test_no_subprocess(self):
        import ast
        import os
        be_dir = os.path.join("backup_manager", "backup_execution")
        for filename in os.listdir(be_dir):
            if not filename.endswith(".py"):
                continue
            filepath = os.path.join(be_dir, filename)
            with open(filepath) as f:
                tree = ast.parse(f.read())
            for node in ast.walk(tree):
                if isinstance(node, ast.Import):
                    for alias in node.names:
                        self.assertNotEqual(alias.name, "subprocess")
                elif isinstance(node, ast.ImportFrom):
                    if node.module and "subprocess" in node.module:
                        self.fail(f"subprocess imported in {filename}")

    def test_no_exec_eval(self):
        import ast
        import os
        be_dir = os.path.join("backup_manager", "backup_execution")
        for filename in os.listdir(be_dir):
            if not filename.endswith(".py"):
                continue
            filepath = os.path.join(be_dir, filename)
            with open(filepath) as f:
                tree = ast.parse(f.read())
            for node in ast.walk(tree):
                if isinstance(node, ast.Call):
                    func = node.func
                    if isinstance(func, ast.Name) and func.id in ("exec", "eval"):
                        self.fail(f"{func.id}() in {filename}")

    def test_pipeline_lifecycle(self):
        """完整管道生命周期"""
        class MockPipeline(BackupExecutionPipeline):
            def prepare(self, request):
                pass
            def execute(self, request):
                return BackupExecutionResult(
                    backup_id=request.backup_id,
                    status=BackupExecutionStatus.COMPLETED,
                    success=True, message="ok",
                )
        pipeline = MockPipeline()
        req = BackupExecutionRequest(
            backup_id="b-001", operation_id="op-001",
            job_id="j-001", execution_id="exec-001", adapter_id="linux-001",
        )
        pipeline.prepare(req)
        result = pipeline.execute(req)
        self.assertTrue(result.success)
        self.assertEqual(result.status, BackupExecutionStatus.COMPLETED)


# ============================================================================
# Extended Tests
# ============================================================================

class TestBackupExecutionExtended(unittest.TestCase):
    """扩展测试"""

    def test_error_hierarchy(self):
        self.assertTrue(issubclass(InvalidBackupExecutionRequestError, BackupExecutionError))
        self.assertTrue(issubclass(BackupExecutionConflictError, BackupExecutionError))
        self.assertTrue(issubclass(BackupExecutionUnavailableError, BackupExecutionError))

    def test_request_repr_no_secrets(self):
        req = BackupExecutionRequest(
            backup_id="b-001", operation_id="op-001",
            job_id="j-001", execution_id="exec-001", adapter_id="linux-001",
        )
        r = repr(req)
        for term in ["password", "secret", "token"]:
            self.assertNotIn(term, r.lower())

    def test_result_repr_no_secrets(self):
        result = BackupExecutionResult(
            backup_id="b-001", status=BackupExecutionStatus.COMPLETED,
            success=True, message="ok",
        )
        r = repr(result)
        for term in ["password", "secret", "token"]:
            self.assertNotIn(term, r.lower())

    def test_request_preserves_all_fields(self):
        req = BackupExecutionRequest(
            backup_id="b-001", operation_id="op-001",
            job_id="j-001", execution_id="exec-001", adapter_id="linux-001",
        )
        self.assertEqual(req.backup_id, "b-001")
        self.assertEqual(req.operation_id, "op-001")
        self.assertEqual(req.job_id, "j-001")
        self.assertEqual(req.execution_id, "exec-001")
        self.assertEqual(req.adapter_id, "linux-001")

    def test_executor_returns_result(self):
        class MockExecutor(BackupExecutor):
            def execute(self, request):
                return BackupExecutionResult(
                    backup_id=request.backup_id,
                    status=BackupExecutionStatus.COMPLETED,
                    success=True, message="ok",
                )
        executor = MockExecutor()
        req = BackupExecutionRequest(
            backup_id="b-001", operation_id="op-001",
            job_id="j-001", execution_id="exec-001", adapter_id="linux-001",
        )
        result = executor.execute(req)
        self.assertIsInstance(result, BackupExecutionResult)

    def test_pipeline_prepare_then_execute(self):
        class MockPipeline(BackupExecutionPipeline):
            def __init__(self):
                self.prepared = False
            def prepare(self, request):
                self.prepared = True
            def execute(self, request):
                return BackupExecutionResult(
                    backup_id=request.backup_id,
                    status=BackupExecutionStatus.COMPLETED,
                    success=self.prepared, message="ok" if self.prepared else "not prepared",
                )
        pipeline = MockPipeline()
        req = BackupExecutionRequest(
            backup_id="b-001", operation_id="op-001",
            job_id="j-001", execution_id="exec-001", adapter_id="linux-001",
        )
        pipeline.prepare(req)
        result = pipeline.execute(req)
        self.assertTrue(result.success)

    def test_error_messages_safe(self):
        try:
            raise BackupExecutionError("test")
        except BackupExecutionError as e:
            msg = str(e)
            for term in ["password", "secret", "token", "command"]:
                self.assertNotIn(term, msg.lower())

    def test_conflict_error_message(self):
        exc = BackupExecutionConflictError("test-backup")
        self.assertIn("test-backup", str(exc))

    def test_unavailable_error_message(self):
        exc = BackupExecutionUnavailableError("service down")
        self.assertIn("service down", str(exc))

    def test_request_whitespace_id_rejected(self):
        with self.assertRaises(InvalidBackupExecutionRequestError):
            BackupExecutionRequest(
                backup_id="   ", operation_id="op-001",
                job_id="j-001", execution_id="exec-001", adapter_id="linux-001",
            )

    def test_request_whitespace_operation_id_rejected(self):
        with self.assertRaises(InvalidBackupExecutionRequestError):
            BackupExecutionRequest(
                backup_id="b-001", operation_id="   ",
                job_id="j-001", execution_id="exec-001", adapter_id="linux-001",
            )

    def test_request_whitespace_job_id_rejected(self):
        with self.assertRaises(InvalidBackupExecutionRequestError):
            BackupExecutionRequest(
                backup_id="b-001", operation_id="op-001",
                job_id="   ", execution_id="exec-001", adapter_id="linux-001",
            )

    def test_request_whitespace_execution_id_rejected(self):
        with self.assertRaises(InvalidBackupExecutionRequestError):
            BackupExecutionRequest(
                backup_id="b-001", operation_id="op-001",
                job_id="j-001", execution_id="   ", adapter_id="linux-001",
            )

    def test_request_whitespace_adapter_id_rejected(self):
        with self.assertRaises(InvalidBackupExecutionRequestError):
            BackupExecutionRequest(
                backup_id="b-001", operation_id="op-001",
                job_id="j-001", execution_id="exec-001", adapter_id="   ",
            )

    def test_result_empty_message_accepted(self):
        result = BackupExecutionResult(
            backup_id="b-001", status=BackupExecutionStatus.COMPLETED,
            success=True, message="",
        )
        self.assertEqual(result.message, "")

    def test_result_all_statuses(self):
        for status in BackupExecutionStatus:
            result = BackupExecutionResult(
                backup_id="b-001", status=status,
                success=True, message="ok",
            )
            self.assertEqual(result.status, status)

    def test_pipeline_returns_result(self):
        class MockPipeline(BackupExecutionPipeline):
            def prepare(self, request):
                pass
            def execute(self, request):
                return BackupExecutionResult(
                    backup_id=request.backup_id,
                    status=BackupExecutionStatus.COMPLETED,
                    success=True, message="ok",
                )
        pipeline = MockPipeline()
        req = BackupExecutionRequest(
            backup_id="b-001", operation_id="op-001",
            job_id="j-001", execution_id="exec-001", adapter_id="linux-001",
        )
        result = pipeline.execute(req)
        self.assertIsInstance(result, BackupExecutionResult)


if __name__ == "__main__":
    unittest.main()
